...
app.post("/tap", async (req, res) => {
    try {
        let { accountId, cardUid, amount, memo, merchant } = req.body;
        ...
        if (!accountId)
            return res.status(400).json({ error: "accountId or cardUid required" });
        // de-dupe
        const now = Date.now();
        const prev = lastTapAt.get(accountId) || 0;
        if (now - prev < 2000)
            return res.status(429).json({ error: "duplicate tap" });
...
app.post("/pay-credit", async (req, res) => {
    try {
        let { accountId, cardUid, amount } = req.body;
        if (typeof amount !== "number" || amount <= 0) {
            return res.status(400).json({ error: "positive amount required" });
        }
        if (!accountId && cardUid) {
...
app.post("/register", async (req, res) => {
    try {
        ...
        if (!accountId || !name)
            return res.status(400).json({ error: "accountId (or id) and name required" });
        const ref = db.collection("accounts").doc(accountId);
...
app.post("/link-card", async (req, res) => {
    try {
        const { cardUid, accountId } = req.body;
        if (!cardUid || !accountId)
            return res.status(400).json({ error: "cardUid and accountId required" });
...
app.post("/unlink-card", async (req, res) => {
    try {
        ...
        if (!cardUid)
            return res.status(400).json({ error: "cardUid required" });
...
app.post("/admin/payroll/pay-one", async (req, res) => {
    try {
        ...
        await db.runTransaction(async (t) => {
            ...
            if (amt <= 0)
                throw new Error("No stipend or amount is zero");
...
app.post("/transfer", async (req, res) => {
    try {
        let { accountId, cardUid, from, to, amount } = req.body;
